...
# SCI Table
IR_SCI_Table <- select(IR_SCI, BenSampID:SCI, method, FamTotTaxa:`CPMI65-CHOWAN`) 
...
sites_sf <- distinct(IR_SCI, StationID, .keep_all = T) %>%
  ...
  select(BenSampID, StationID, Lat, Long, StreamName, Basin, Subecoregion,
         Location, Region, EcoRegion, County) %>%# drop data to avoid any confusion
...
leaflet(shared_sites_sf) %>% 
  ...
  addCircleMarkers(data=shared_sites_sf,color='yellow', fillColor='blue', radius = 5,
                   ...
                   popup=~paste(sep='<br>',
                                ...
                                paste(strong('Subecoregion : '), Subecoregion),
...
datatable(shared_IR_decision, extensions = 'Buttons',  rownames = FALSE, options = list(dom= 'Bt', buttons=list('copy')))
```
